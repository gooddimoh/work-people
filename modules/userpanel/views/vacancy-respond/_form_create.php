...
/* @var $model_vacancy app\models\Vacancy */
/* @var $my_resume_list app\models\Resume */
    ...
    <div class="container">
        <div class="registration__inner">
            <?php $form = ActiveForm::begin([
                ...
                <div class="registration__input-bl">
                    <div class="registration__input-title">
                        <?= Yii::t('vacancy', 'You can attach a resume to your job posting') ?>
                    </div>
                    <input v-validate="vv.model_vacancy_respond.resume_id" name='VacancyRespond[resume_id]' type="hidden" v-model="vacancy_respond.resume_id">
                    <nice-select :options="my_resume_list" name="VacancyRespond[resume_id]`" class="select" v-model="vacancy_respond.resume_id">
                        <option v-for="item in my_resume_list" :value="item.id" v-cloak>
                            {{ item.title }}
                        </option>
                    </nice-select>
                    <span v-show="errors.has('VacancyRespond[resume_id]')" class="validation-error-message" v-cloak>
                        {{ errors.first('VacancyRespond[resume_id]') }}
                    </span>
                </div>
                <div class="registration__input-bl">
...
new Vue({
  ...
  data: {
    my_resume_list: [
        ...
        <?php foreach( $my_resume_list as $resume) { ?>
        <?php
            $category_jobs = $resume->getCategoryJobs()->asArray()->all();
        ...
        {
            id: '<?= $resume->id ?>',
            title: '#<?= $resume->id ?> - <?= $category_job_names ?> (<?= Html::encode($resume->getFullName()) ?>)',
        },
        <?php } ?>
    ],
    //-------
    ...
    vv: {
        model_vacancy_respond: {
            resume_id: '<?= VeeValidateHelper::getVValidateString($this, $model, 'resume_id') ?>',
  ...
  methods: {
    onSubmit () {
        ...
        this.$validator.validate().then(valid => {
            ...
            let post_data = {
                // 'VacancyRespond[resume_id]': this.$data.vacancy_respond.resume_id,
                'VacancyRespond[message]': this.$data.vacancy_respond.message,
            };

            if(this.$data.vacancy_respond.resume_id != '') {
                post_data['VacancyRespond[resume_id]'] = this.$data.vacancy_respond.resume_id
            }
