...
$this->title = Yii::t('vacancy', 'Import Excel (*.xls) file');
...
<div id="appImportExcel" class="vacancy-form">
    ...
    <div style="margin-top: 30px;">
        ...
        <div class="validation-error-message" v-if="file_format_error" v-cloak><?= Yii::t('vacancy', 'Invalid file format') ?></div>
        <div class="row" v-cloak>
            <div class="col">
                <button class="btn btn-success" v-if="getSuccessCount > 0 && !import_complete" v-on:click="completeImport">Import into DB [{{getSuccessCount}}] Vacancy</button>
            </div>
        </div>
        <div class="row" v-if="import_complete" v-cloak>
            <div class="col">
                <span class="text-info">{{getSuccessCount}} <?= Yii::t('vacancy', 'Vacancy import complete!') ?></span>&nbsp;
                <a href="<?= Url::toRoute(['index']) ?>"><?= Yii::t('vacancy', 'Go to vacancy list') ?></a>
            </div>
        </div>
