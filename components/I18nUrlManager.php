...
class I18nUrlManager extends UrlManager
{
    ...
    public function parseRequest($request)
    {
        if ($this->enablePrettyUrl) {
            ...
            } else { // setup default language form user agent
				$pattern = substr(Yii::$app->request->headers->get('Accept-Language'), 0, 2);
				$locale = array_values(preg_grep("/{$pattern}/i", $this->languages));
				if(count($locale) > 0)
					Yii::$app->language = $locale[0];
			}
        } else {
